	...
	public function accessRules()
	{
		...
		*/
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('view','list','wherelooking','api'),
				'users'=>array('*'),
			),
...
$portfolios=Yii::app()->cache->get($portfolio);
		    	...
		    	foreach($portfolios as $portfolio)
		    	{
		    		...
		    		if($portfolio->min_dollar <= $buyer->pof_amount)
		    		{
			    		...
			    		if($score >= 1 && Users::model()->findByPk($portfolio->create_user_id)->getVoteSum() >= 0 && Users::model()->findByPk($buyer->create_user_id)->getVoteSum() >= 0 && Users::model()->findByPk($portfolio->create_user_id)->id != 92)
			    		{	
			    			$limit += 1;
