...
class BuyerController extends Controller
{
	...
	public function accessRules()
	{
		...
		*/
			...
			array('allow',  // allow all users
				'actions'=>array('tirekicker','interview','letterofintent', 'buyerwidget','matches'),
				'users'=>array('*'),
			),
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
	...
	public function actionTireKicker($id)
	{
		
	$model = new Buyer;

	$buyer = $model->findByAttributes(array('id'=>$id));
	//print_r($buyer);
	
	if ($buyer->status_id == 1) {

	//status_id 2 is Not Matching
	$buyer->status_id = 2;
	
	$buyer->update(false);
	
	Yii::app()->user->mp_track("Tire Kicker");
	Yii::app()->user->mp_increment("Tire Kicker");
	...
	Yii::app()->user->mp_track("Urly Bird");
	Yii::app()->user->mp_increment("Urly Bird");
		
	}
	
	$this->redirect("https://www.accesstheflock.io/buyer/$id");
...
if($votes_for_renturly >= 0 && $asset_type_match > 0 && $location_match > 0 && $votes_for_fistfuls >= 0)
			    		{	
