				...
				array (
				  ...
				  'removeButtonHtmlOptions' => array('class'=>'pull-right'), //optional
						'fields'=>array( //required
								...
								(
										'label'=>array(
												'title'=>'Location (USA is Nationwide)',
										),
										'field'=>array(
...
const input = document.getElementById("yw1_WhereLooking_1_location");

//yw1_WhereLooking_2__location
document.getElementsByClassName("clone-row")[1].addEventListener("click" ,function(){

var locations = document.getElementsByClassName("location");

order = Array.from(locations);

order.forEach(addLocation);

function addLocation(item, index)
{
	console.log(item);
	const autocomplete2 = new google.maps.places.Autocomplete(item, options);
}
